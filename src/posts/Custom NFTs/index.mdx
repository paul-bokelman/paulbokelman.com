...
## Specifically we will be creating a smart contract that allows the current user to mint a NFT directly to their wallet. We will also learn how to customize these NFTs to your liking.
...
<!-- <F files={["Migrations.sol", "SimpleStorage.sol"]}>contracts</F> -->

## You may have noticed by now that we a contracts folder outside of our client folder that contains two Solidity (.sol) files. The only file we are going to change in this folder is the `SimpleStorage.sol`, this is where our minting logic will happen.
...
## In our <F files={["1_initial_migration.js", "2_deploy_contracts.js"]}>Migrations</F> folder our `2_deploy_contracts.js` should look like this. And our `InitWeb3.js` file should look like this.
