...
tags: ["detailed", "react", "detailed", "react"]
...
## Our `state.js` is now finished so we can close it and move onto our component that will utilize our custom `useStore` hook.

## Create a file called `Toggle.js` in the <F folder={folders.components}>components</F> folder. This file will be used to check if the current state of `dark` is <Bool>true</Bool> or <Bool>false</Bool> and conditionally render the light and dark icons that both have the ability to change the theme with the `toggleDarkMode` function that we created in the `state.js`.
...
## Now we need to create a <F folder={folders.elements}>elements</F> folder inside this folder we need to create a `index.js` file as well as a `Icon.js` file which will hold all of our icon styles.
...
## Now lets get to our `Icon.js`. Using styled-components is very easy, the first thing we are going to do is import the library then export a variable called `IconWrapper` and set it equal to `styled.div`and end with 2 backticks.
